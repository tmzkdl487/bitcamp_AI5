...
for epoch in range(1, num_epochs):
    model.train()
    epoch_loss = 0
    train_progress = tqdm(train_loader, desc=f"Epoch {epoch}", leave=False)
...
torch.save(model.state_dict(), save_path + 'LSTM_no_c0.pth')
...
with torch.no_grad():
    model.load_state_dict(torch.load(save_path+'LSTM_no_c0.pth', map_location=DEVICE))
    eval_progress = tqdm(train_loader, desc="Evaluating", leave=False)
              
    for x_test, y_test in eval_progress:
...
#  len : 937
# R2 스코어 : 0.9736675312613838
<<<<<<< HEAD
# total_loss :  284.1192234667917
=======
# total_loss :  284.1192234667917
