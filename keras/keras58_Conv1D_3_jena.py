# keras56_Bidirectional3_jena.py

from tensorflow.keras.models import Sequential
from tensorflow.keras.layers import Dense, LSTM, Dropout, Flatten, Bidirectional, Conv1D, MaxPool1D
...
# 2. 모델
model = Sequential()
model.add(Conv1D(64, kernel_size=2, input_shape=(144, 15)))   # timesteps, features / activation='tanh'
model.add(MaxPool1D())
model.add(Conv1D(256, 3))
model.add(MaxPool1D())
model.add(Conv1D(524, 4))
# model.add(MaxPool1D())
...
################# # Conv1 2개씀 ######################################
# 로스는 :  5.1358513832092285 / RMSE :  3.389447427976532  / 걸린시간 :  52.9 초  <- Conv1 2개씀/ patience = 10
# 로스는 :  2.637876510620117  / RMSE :  2.5655612480254306 / 걸린시간 :  92.52 초 <- model.add(MaxPool1D()) 넣음
# 로스는 :  2.853013277053833  / RMSE :  2.644060748786542  / 걸린시간 :  78.91 초
# 로스는 :  14.30124282836914  / RMSE :  4.278859498268393  / 걸린시간 :  47.71 초
# 로스는 :  5.172813415527344  / RMSE :  2.1425468209591956 / 걸린시간 :  110.18 초
# 로스는 :  5.886221408843994  / RMSE :  2.557825091559655  / 걸린시간 :  99.12 초
# 로스는 :  9.808611869812012  / RMSE :  2.900556729748901  / 걸린시간 :  129.3 초  <- 모델 3자리로 바꿈.
# 로스는 :  70.46011352539062  / RMSE :  12.404345929294218 / 걸린시간 :  681.93 초 <- 모델 4자리
# 로스는 :  19.09656524658203  / RMSE :  4.152864675895462  / 걸린시간 :  221.29 초
# 로스는 :  70.46057891845703  / RMSE :  12.407166677256495 / 걸린시간 :  773.95 초